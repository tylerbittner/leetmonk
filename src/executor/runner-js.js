...
// Harness inlined so it survives electron-vite bundling
...
export async function execRunnerJs({ code, cases, problem }) {
  const functionName = detectFunctionName(code, problem)
  if (!functionName) {
    return {
      results: [],
      error: 'Could not detect function name. Make sure your code has a top-level function definition (function functionName(...)).'
    }
  }

  const tmpId = randomUUID()
  const tmpFile = join(tmpdir(), `leetmonk_js_${tmpId}.js`)
  ...
  return new Promise((resolve) => {
    ...
    child.on('close', () => {
      ...
      try {
        ...
        resolve({ results: parsed.results, error: parsed.error || null, stderr: stderr || null })
      } catch {
    ...
    child.on('error', (err) => {
      ...
      if (err.code === 'ENOENT') {
        resolve({
          results: [],
          error: 'Node.js not found. Please ensure `node` is in your PATH.'
        })
      } else {
